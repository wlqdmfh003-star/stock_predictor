...
class DataFetcher:
    ...
    def fetch_all_parallel(self, max_workers=4, per_task_timeout=30):
        ...
        - `per_task_timeout`: 개별 티커 작업 타임아웃(초)
        """
        ...
        # 간단한 타임아웃/취소: 오래 걸리는 future는 취소하고 실패로 기록
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            while pending:
                ...
                for f in list(done):
                    ...
                    pending.discard(f)
                    completed += 1
                    if completed % 10 == 0 or completed % 30 == 0:
                ...
                for f in list(pending):
                    if now - start_times.get(f, now) > per_task_timeout:
                        code = future_to_code.get(f)
                        print(f"   [TIMEOUT] {code} exceeded {per_task_timeout}s — cancelling")
                        try:
                            f.cancel()
                        except Exception:
                            pass
                        ...
                        completed += 1
                        try:
                            self._log_failed([future_to_code.get(f)])
                        except Exception:
                            pass
                        if completed % 10 == 0 or completed % 30 == 0:
                            print(f"   가격 다운로드: {completed}/{len(tickers)}")
        ...
        for rec in records:
            ...
            try:
                ...
                rows.append({
                    ...
                    "current_price":close,
                    "open":float(last.get('open',0)),"high":float(last.get('high',0)),"low":float(last.get('low',0)),
                    "volume":vol,"volume_bil":vol*close/1e8,"market_cap":cap,
                    "ohlcv":ohlcv,"ohlcv_weekly":weekly,"ohlcv_monthly":monthly,
                    "prev_close":prev_close,"prev_high":float(prev.get('high',0)),"prev_low":float(prev.get('low',0)),
        ...
        if "market_cap" in df.columns and self.min_market_cap > 0:
            ...
            if after >= 10:
                ...
                print(f"  [WARN] 필터 후 {after}개로 너무 적음 → 원본 {before}개 유지")
